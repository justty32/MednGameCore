...
> 定位：workflows/idea/data-driven/ 目錄的索引——資料驅動、內容管線與模組化各主題檔的關係、優先序總表、建議推進順序。
...
與 `workflows/idea/gameplay/` 是姊妹目錄：gameplay 談「做什麼機制」，
本目錄談「機制的參數與內容放哪裡、怎麼載入、怎麼演進」。

這些是**想法，不是定案 spec**。動工前仍應走 brainstorming 收斂成
`workflows/specs/archive/` 的正式設計文件。
...
- 設計真相層：`workflows/specs/archive/2026-06-03-gamecore-zone-design.md`
